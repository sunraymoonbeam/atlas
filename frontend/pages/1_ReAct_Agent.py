...
def main():
    ...
    if not ss.collection_name:
        st.warning(
            ...
            "to begin chatting; Atlas uses it to ground its answers in your documents."
        )
        return
    ...
    st.write(
        """
        This chatbot helps you explore Company X’s services by reasoning 
        step by step, deciding which tools to call (document retrieval, web search, etc.).
        Feel free to ask anything related to Company X’s services.
        """
    )
    ...
    user_input = st.chat_input(
        placeholder="e.g. How do I sell my car through Company X? Or what are the latest financing rates?"
    )
    if not user_input:
        return
    ...
    status_placeholder.markdown("🤔 Atlas is thinking...")
