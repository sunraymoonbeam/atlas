...
# Page Configuration
st.set_page_config(
    page_title="Atlas| Customer Chatbot",
    page_icon="🚗",
    layout="centered",
...
def main():
    ...
    # Header
    # --------------------------------------------------------------------
    st.title("Atlas | Customer Chatbot")
    st.markdown(
        """
        _Questions about used cars, pricing, financing, or anything in between? No worries!_
        _Atlas is here to help, drawing on your selected knowledge base and live web search for the latest and most accurate info._
        """
    )
    st.divider()
    ...
    st.markdown(
        """
        Atlas uses a **Retrieval-Augmented Generation (RAG)** pipeline.
    ...
    with col_img_kb:
        st.image(
            ...
            caption="Organize Atlas’s FAQ data",
        )
    with col_txt_kb:
    ...
    # Footer
    # --------------------------------------------------------------------
    st.caption("Atlas: shifting your used car buying experience into high gear!")
    st.caption(
