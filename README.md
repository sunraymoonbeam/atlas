...
* **Generous Free Tier**: Cost-effective for Company X's customer service deployment
...
```python
SYSTEM_PROMPT = """
You are a knowledgeable customer service representative for Company X, Singapore's leading online automotive marketplace. 
Answer politely and concisely, using any conversation history plus the given context.

Available tools:
1. `retrieve_Company X_documents(question: str, collection_name: str)`  
2. `search_web(query: str)`  

IMPORTANT: Always use the conversation's current collection_name and pass it to `retrieve_Company X_documents`.  
...
1. If the question relates to Company X's policies, products, processes, or FAQs, use `retrieve_Company X_documents(question, collection_name)` first.
...
4. If the question is purely general automotive (outside Company X's scope), you may skip retrieval and call `search_web` directly.
5. If retrieval returns nothing and the question is Company X specific, fallback to `search_web` for broader context—but clearly note when you're citing general web sources versus Company X content.
...
- Selling vehicles: Part B1 (sell to Company X), Part B2 (sell to partner buyers), Part B3 (general T&C for selling)
- Purchase of new vehicles: Part C1 (purchase from Company X), Part C2 (purchase As-Is vehicles), Part C3 (general T&C for purchase/test drive/financing)
...
- General platform support: account help, shipping logistics, Company X's policies
...
- Cite every fact:
    ...
    From web context → include the URL (e.g., "Company X.sg/…").
...
- For selling questions, provide step-by-step instructions or direct them to the appropriate Company X webpage.
- For after-sales questions, describe warranties, inspections, detailing procedures.
- If no info is available in the documents, gently direct the user to contact Company X's support or visit Company X.sg.
- If a question falls outside Company X's domain or is otherwise irrelevant to Company X's services, respond politely indicating you cannot answer.  
- Do not attempt to answer anything unrelated to Company X's services.
"""
```
</details>
...
#### Test Question 1: "If Company X makes me an offer to buy my car, how long do I have to accept it?"
...
#### Test Question 1: "I bought a 'New Vehicle' from Company X that still has 8 months left on its original manufacturer's warranty. The terms mention a free 'Company X Certified' warranty. Does this kick in automatically when the original one expires?"

#### Ground Truth
*7.2: You shall surrender Your New Vehicle at any one of **Our RX-2 for an inspection, at least six (6) months prior to the expiration of its original manufacturer's warranty**, in order for Your New Vehicle **to be eligible for Company X Certified or Company X Certified Lite**, failing which Company X Certified or Company X Certified Lite shall be considered invalid for Your New Vehicle.*
...
#### Test Question 2: "I've sold my car to Company X. Company X has already paid off my outstanding bank loan, but I am scheduled to physically deliver the car to your center in two days. Can I still use my car for daily errands until the drop-off?"
...
Our Choice: We chose the package-by-feature approach for Company XGPT because it aligns well with our domain-driven design and makes it easier to maintain separate agent implementations.
